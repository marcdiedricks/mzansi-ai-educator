...
import { getUserProgress, getLevel2TestProgress, isLevel2TestPreviewEnabled } from '../lib/progress';
import { lessons } from '../lib/curriculum/data';
import { LEVEL_TWO_MODULES } from '../lib/programmeLevels';

export function Progress() {
  const [progress, setProgress] = useState(getUserProgress());
  const [testProgress, setTestProgress] = useState(getLevel2TestProgress());
  const [testPreview, setTestPreview] = useState(isLevel2TestPreviewEnabled());

  useEffect(() => {
    const handleUpdate = () => {
      setProgress(getUserProgress());
      setTestProgress(getLevel2TestProgress());
  ...
  const level2Visible = level1Complete || testPreview;
  ...
  return (
    <div className="p-6 space-y-6 pb-24">
      ...
      {testPreview && !level1Complete && <div className="bg-amber-50 border-2 border-amber-300 rounded-xl p-3 flex gap-2 text-xs text-amber-900"><Eye className="w-4 h-4 shrink-0" /><span><strong>TEST PREVIEW ACTIVE.</strong> Level 2 test activity is stored separately. Level 1 progress, badges and certificates are unchanged.</span></div>}
      ...
      <div className={`rounded-2xl p-6 border-2 ${level2Visible ? 'bg-white border-[#2D3E50]' : 'bg-gray-50 border-gray-200 opacity-70'}`}>
        ...
        {testPreview && !level1Complete && completed2 === level2Ids.length && <div className="mt-3 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs font-bold text-amber-900">Level 2 test preview complete. Learner progression remains locked because Level 1 is not complete.</div>}
      </div>

      <div className="grid grid-cols-2 gap-3"><div className="bg-white border-2 border-[#E2E8F0] rounded-xl p-5 text-center"><BookOpen className="w-6 h-6 text-[#2D3E50] mx-auto mb-2" /><div className="text-2xl font-bold text-[#1A202C]">{testPreview && !level1Complete ? completed1 : completed1 + completed2}</div><div className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Learner Lessons Completed</div></div><div className="bg-white border-2 border-[#E2E8F0] rounded-xl p-5 text-center"><Target className="w-6 h-6 text-[#E67E22] mx-auto mb-2" /><div className="text-2xl font-bold text-[#1A202C]">{progress.completedLabs.length}</div><div className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Activities Done</div></div></div>

      {testPreview && !level1Complete && <div className="bg-white border-2 border-amber-200 rounded-xl p-5 text-center"><div className="text-2xl font-bold text-amber-800">{completed2}</div><div className="text-[10px] font-bold uppercase tracking-wider text-amber-700">Level 2 Test Lessons Completed</div></div>}

      <div className="bg-white border-2 border-[#E2E8F0] rounded-xl p-5 space-y-3"><div><div className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Level 1 Average Quiz Score</div><div className="text-xl font-bold text-[#1A202C]">{average(level1QuizScores as Array<[string, number]>)}%</div></div><div className="border-t border-gray-200 pt-3"><div className="text-[10px] font-bold uppercase tracking-wider text-gray-400">{testPreview && !level1Complete ? 'Level 2 Test Quiz Average' : 'Level 2 Average Quiz Score'}</div><div className="text-xl font-bold text-[#1A202C]">{level2Visible ? `${average(level2QuizScores as Array<[string, number]>)}%` : 'Locked'}</div></div></div>
