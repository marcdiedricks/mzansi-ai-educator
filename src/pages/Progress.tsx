...
import { getUserProgress, getLevel2TestProgress, getLevel3TestProgress, getLevel4TestProgress, isLevel2TestPreviewEnabled, isLevel3TestPreviewEnabled, isLevel4TestPreviewEnabled } from '../lib/progress';
...
export function Progress() {
  ...
  const completed4 = level4Ids.filter((id) => level4Progress.completedLessons.includes(id)).length;
  ...
  const level4Visible = level3RealComplete || level4Preview;
  ...
  const level4QuizScores = Object.entries(level4Progress.quizScores).filter(([id]) => level4Ids.includes(id));
  ...
  return (
    <div className="p-6 space-y-6 pb-24">
      ...
      {level4Preview && !level3RealComplete && <div className="bg-amber-50 border-2 border-amber-300 rounded-xl p-3 flex gap-2 text-xs text-amber-900"><Eye className="w-4 h-4 shrink-0" /><span><strong>LEVEL 4 TEST PREVIEW ACTIVE.</strong> Level 4 test activity is isolated from learner progress and certificates.</span></div>}
      ...
      <div className={`rounded-2xl p-6 border-2 ${level4Visible ? 'bg-white border-[#2D3E50]' : 'bg-gray-50 border-gray-200 opacity-70'}`}>
        <div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-orange-50 rounded-xl flex items-center justify-center"><Target className="w-6 h-6 text-[#E67E22]" /></div><div><h2 className="text-lg font-bold text-[#2D3E50]">Level 4 · Lead & Solve</h2><p className="text-xs text-gray-500">AI Innovation & Problem-Solving{level4Preview && !level3RealComplete ? ' · Isolated Test Preview' : ''}</p></div></div>
