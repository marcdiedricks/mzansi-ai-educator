    ...
    <div className="p-4 sm:p-6 space-y-6 pb-24">
      ...
      {testPreview && !level1Complete && (
        <div className="bg-amber-50 border-2 border-amber-300 rounded-xl p-3 text-xs text-amber-900">
          <strong>TEST PREVIEW ACTIVE.</strong> Level 2 is open for build verification only. Preview completions and quiz scores are stored separately from learner progress.
        </div>
      )}
            ...
            <div key={level.id} className={`rounded-2xl border-2 p-4 ${unlocked ? 'bg-white border-[#2D3E50]' : 'bg-[#F8F9FA] border-[#E2E8F0]'}`}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {unlocked && completed && <p className={`text-[10px] font-bold uppercase tracking-wider mt-2 ${level.id === 2 && testPreview && !level1Complete ? 'text-amber-700' : 'text-emerald-700'}`}>{level.id === 2 && testPreview && !level1Complete ? 'Test preview complete · learner gate unchanged' : 'Completed on this device'}</p>}
                  {level.id === 2 && testPreview && !level1Complete && !completed && <p className="text-[10px] font-bold uppercase tracking-wider text-amber-700 mt-2">Test preview · learner gate unchanged</p>}
                </div>
              </div>
            </div>
          );
      ...
      <section className={`border-2 rounded-2xl overflow-hidden ${level2Accessible ? 'bg-white border-[#2D3E50]' : 'bg-white border-[#E2E8F0]'}`}>
        <div className={`${level2Accessible ? 'bg-[#2D3E50] text-white' : 'bg-[#F8F9FA] text-[#1A202C]'} p-5 border-b-2 border-[#E2E8F0]`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className={`text-xs mt-1 ${level2Accessible ? 'text-gray-300' : 'text-gray-600'}`}>{level1Complete ? 'Level 2 is unlocked. Complete the eight practical modules in order.' : testPreview ? 'Build preview is active. Test records are isolated from learner progress.' : 'Complete Level 1 to unlock these eight practical modules.'}</p>
            </div>
        ...
        <div className="divide-y-2 divide-[#E2E8F0]">
          {LEVEL_TWO_MODULES.map((mod, index) => {
            const done = Boolean(mod.lessonId && level2Progress.completedLessons.includes(mod.lessonId));
            return (
              <button key={mod.id} disabled={!level2Accessible || !mod.lessonId} onClick={() => mod.lessonId && navigate(`/learn/${mod.id}/lesson/${mod.lessonId}`)} className="w-full flex items-start p-5 bg-white text-left hover:bg-[#F8F9FA] disabled:opacity-60 disabled:cursor-not-allowed">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-[10px] font-bold uppercase tracking-wider mt-2 ${done ? 'text-emerald-700' : level2Accessible ? 'text-[#2D3E50]' : 'text-gray-400'}`}>{done ? (testPreview && !level1Complete ? 'Test completed' : 'Completed') : level2Accessible ? '20 min lesson · Tap to open' : 'Ready · unlocks after Level 1'}</p>
                </div>
